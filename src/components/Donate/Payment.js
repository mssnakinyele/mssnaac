      ...
      <ul className="col-span-2 flex-col self-start mt-4 cursor-pointer">
        {
          data.map((item) => (
            <li key={item.id} id={item.id} className="li w-full">
              <button id={item.id} type="button" className={`button flex flex-col items-center bg-white justify-center py-14 ${item.id === 1 ? 'rounded-tr-[34px]' : ''} border-b-2 border-[#007300]-600 w-full`} onClick={(e) => toggleDisplay(e)}>
                <div id={item.id} className="border border-[#007300] rounded-full p-3 bg-[#007300] text-white">
                  {item.icon}
                </div>
                <p id={item.id} className="text-[#007300] text-base">{item.text}</p>
              </button>
            </li>
          ))
        }
