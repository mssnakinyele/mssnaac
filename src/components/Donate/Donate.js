    ...
    <>
      ...
      <div className="grid grid-cols-2 min-h-fit">
        ...
        <div className="bg-[#E6F4E6] px-[50px] pt-[50px]">
          <div className="w-1/2">
            <h2>Donate today</h2>
            <p className="text-base">Charity (Sadaqah) extinguishes sin, just as water extinguishes fire</p>
          </div>
          <div className="pt-[50px]">
            ...
            <ul className="flex grow-/i0">
              ...
              data.map((item) => (
                <li key={item.id}>
                  <button type="button" className="border-gray-800 bg-white border-r-1 p-4 hover:bg-[#007300] hover:text-white cursor-pointer" onClick={(e) => setInput(e)}>{item.amount}</button>
                </li>
              ))
}
            </ul>
            <form className="flex flex-col flex-start w-3/4">
              ...
              <p className={`text-sm text-pink-600 pb-3 text-bold ${amount > 100 && amount < 1000001 ? 'hidden' : 'block'}`}>Please conduct transfers ranging from N100 to N1 million per transaction.</p>
